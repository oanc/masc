...
grate scripts/corrections.gr8 $IN/txtfiles $GRAF $TEMP ../meta/MASC2-resource-header.xml
#exit
...
java $OPTS -jar $CONVERT -xces $LOPTS -in=$TEMP -out=$OUT -set=anc -exf="graf:id graf:set" -id=penn -ann=penn -rename="Token=tok @category=msd" -saveAs=penn -exf="graf:set graf:id graf:edge length orth affix kind"
...
java $OPTS -jar $CONVERT -xces $LOPTS -in=$XCES -out=$OUT -set=anc -ann=hepple -id=penn -rename="Token=tok @category=msd" -saveAs=penn -exf="graf:set graf:id graf:edge length orth affix kind"
java $OPTS -jar $CONVERT -xces $LOPTS -in=$XCES -out=$OUT -set=anc -ann=penn -id=penn -rename="Token=tok @category=msd" -saveAs=penn -exf="graf:set graf:id graf:edge length orth affix kind"
