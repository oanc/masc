...
echo opening $TOKEN
java $OPTS -jar $CONVERT -xces $LOPTS -set=anc -ann=penn -id=penn -in=$TOKEN -out=$OUT -saveAs=penn -rename="Token=tok @category=msd @root=base" -exf="length orth affix kind"
